...
internal class FontSizeSettingsPage : BasePager() {
    ...
    internal var followsSystem by observable(true)
    internal var fontScale by observable(FontSizeSettings.DEFAULT_SCALE)
    internal var chatTextSizeSp by observable(ChatBackgroundSettings.DEFAULT_CHAT_TEXT_SIZE_SP)
    ...
    override fun body(): ViewBuilder {
        val ctx = this
        return {
            attr { backgroundColor(ctx.pagePalette().background) }
            SettingsPageHeader(
        ...
        with(container) {
            SettingsCard(
                ...
                View {
                    attr {
                        height(492f)
                        padding(top = 20f, left = 21f, right = 21f, bottom = 28f)
                    }
                    View {
                        attr {
                            flexDirectionRow()
                        ...
                        View {
                            attr {
                                ...
                                padding(top = 10f, left = 13f, right = 13f, bottom = 10f)
                            }
                            Text {
                                attr {
                                    text("帮我预览一下字号大小")
                    ...
                    Text {
                        attr {
                            text("你可以通过拖动下面的滑块来设置字号大小。设置后会改变全局的字号大小，" + "如果在使用过程中遇到问题，可以向我们反馈。")
                            fontSize(ctx.previewTextSize())
    ...
    internal fun sliderProgress(): Float {
        return (fontScale - FontSizeSettings.MIN_SCALE) /
            (FontSizeSettings.MAX_SCALE - FontSizeSettings.MIN_SCALE)
    }

    internal fun scaleForProgress(progress: Float): Float {
        val scaleRange = FontSizeSettings.MAX_SCALE - FontSizeSettings.MIN_SCALE
        return (FontSizeSettings.MIN_SCALE + progress.coerceIn(0f, 1f) * scaleRange).coerceIn(
            FontSizeSettings.MIN_SCALE,
            FontSizeSettings.MAX_SCALE,
        )
    }

    internal fun manualScaleLabel(): String {
        return if (
            (fontScale - FontSizeSettings.DEFAULT_SCALE).let { delta ->
    ...
    internal fun saveAndClose() {
        controller.setFontSize(FontSizeSettings(followsSystem = followsSystem, scale = fontScale))
        closePage()
    }

    internal fun palette(): SettingsPalette = settingsPalette(themeMode)

    internal fun pagePalette(): SettingsPalette {
        val basePalette = palette()
        return if (basePalette === SettingsPalettes.Dark) {
            basePalette
        } else {
            basePalette.copy(
                background = Color(StockChatTheme.COLOR_FFF3F7FA),
                surfaceMuted = Color(StockChatTheme.COLOR_FFF1F1F1),
            )
        }
    }
...
private fun FontSizeSettingsPage.SystemFontControl(container: ViewContainer<*, *>) {
    val ctx = this
    with(container) {
        View {
            attr {
                height(74f)
                padding(top = 16f, left = 18f, right = 16f, bottom = 14f)
                flexDirectionRow()
                alignItemsCenter()
            }
            View {
            ...
            Switch {
                ...
                event { switchOnChanged { isOn -> ctx.followsSystem = isOn } }
            }
        }
    }
}

private fun FontSizeSettingsPage.FontScaleControl(container: ViewContainer<*, *>) {
    val ctx = this
    with(container) {
        View {
            attr {
            ...
            ctx.FontScaleLabels(this)
        }
    }
}

private fun FontSizeSettingsPage.FontScaleLabels(container: ViewContainer<*, *>) {
    val ctx = this
    with(container) {
        View {
            attr {
