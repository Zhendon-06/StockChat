...
internal fun StockDetailPage.LinkedInsightCard(container: ViewContainer<*, *>, quote: StockQuote, prediction: StockPrediction?) {
    val ctx = this
    with(container) {
        View {
            attr {
            ...
            ctx.LinkedInsightHeader(this)
            Text {
                attr {
                    val selectedPoint = ctx.selectedChartPoint()
                    text(
            ...
            ctx.InsightFocusPicker(this)
            Text {
                attr {
                    text(ctx.linkedInsightText(quote, prediction, ctx.selectedChartPoint()))
            ...
            prediction?.conclusions?.forEach { conclusion ->
                val history = ctx.predictionHistory().map { FinancialPoint(it.timestamp, it.close, it.close, it.close, it.close) }
                val resolution = resolveDetailEvidence(conclusion.reference, quote.symbol, MarketPeriod.DAY, quote.updatedAt, history)
                ChartConclusionCard(conclusion, resolution) {
                    ...
                    val currentHistory =
                        ctx.predictionHistory().map { FinancialPoint(it.timestamp, it.close, it.close, it.close, it.close) }
                    val current =
                        resolveDetailEvidence(conclusion.reference, quote.symbol, MarketPeriod.DAY, quote.updatedAt, currentHistory)
            ...
            ctx.ContinueStockConversation(this, quote)
        }
    }
}

internal fun StockDetailPage.selectedChartPoint(): SelectedChartPoint? {
    ...
    val label =
        if (chartShowingPrediction && index >= historyCount) {
            "模型估计 ${predictionPlot().points[index].label}"
        } else {
            "历史收盘 ${predictionPlot().points[index].label}"
        }
    return SelectedChartPoint(label = label, price = financialNumber(points[index]), value = points[index], index = index)
}
...
internal fun StockDetailPage.linkedInsightText(
    ...
    return when (insightFocus) {
        DetailInsightFocus.TREND -> trendInsightText(quote, selectedPoint)
        DetailInsightFocus.PREDICTION -> prediction?.rationale ?: "点击走势图右上角“AI 预测”，模型会先读取真实历史行情；只有成功返回并通过校验后，才会显示虚线预测区间。"
...
private fun StockDetailPage.LinkedInsightHeader(container: ViewContainer<*, *>) {
    val ctx = this
    with(container) {
        View {
            attr {
...
private fun StockDetailPage.InsightFocusPicker(container: ViewContainer<*, *>) {
    val ctx = this
    with(container) {
        View {
            attr {
            ...
            DetailInsightFocus.values().forEach { focus ->
                View {
                    ...
                    event { click { ctx.insightFocus = focus } }
                    Text {
                        attr {
                            text(focus.label)
                            fontSize(scaledFontSize(12f))
...
private fun StockDetailPage.ContinueStockConversation(container: ViewContainer<*, *>, quote: StockQuote) {
    val ctx = this
    with(container) {
        View {
            attr {
            ...
            event { click { ctx.openChatWithStock(quote, ctx.selectedChartPoint()) } }
            Text {
                attr {
                    text("围绕此标的继续追问 AI  ›")
                    fontSize(scaledFontSize(12f))
...
private fun StockDetailPage.trendInsightText(quote: StockQuote, selectedPoint: SelectedChartPoint?): String {
    return if (selectedPoint == null) {
        ...
        "选中${selectedPoint.label}，价格约 ${selectedPoint.price}，处于近段走势${position}，${movement}。" +
            "结合当前涨跌${quote.changePercent}，这个节点更适合用来观察趋势是否延续，不宜只凭单点下结论。"
    }
}
