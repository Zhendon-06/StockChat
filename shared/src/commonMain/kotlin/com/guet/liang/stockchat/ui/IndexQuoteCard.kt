...
import com.guet.liang.kuiklychart.ui.AreaChart
...
internal fun ViewContainer<*, *>.IndexQuoteCard(quote: StockQuote, scale: Float, onClick: () -> Unit) {
    ...
    View {
        ...
        event { click { onClick() } }
        QuoteCardHeader(quote, scale)
        QuoteCardPrice(quote, movementColor, scale)
        Text {
            attr {
        ...
        if (values.size > 1) {
            IndexQuoteChart(values, movementColor, scale)
        } else {
            Text {
                attr {
                    text("历史数据不足 · 点击详情重新加载")
                    fontSize(11f * scale)
                    color(StockChatTheme.textTertiary)
                    marginTop(10f * scale)
                }
...
private fun ViewContainer<*, *>.QuoteCardHeader(quote: StockQuote, scale: Float) {
    View {
...
private fun ViewContainer<*, *>.QuoteCardPrice(quote: StockQuote, movementColor: Color, scale: Float) {
    View {
...
private fun ViewContainer<*, *>.IndexQuoteChart(
    values: List<Float>,
    movementColor: Color,
    scale: Float,
) {
        ...
        chart {
            ...
            axes {
                ...
                y {
                    includeZero = false
                    tickCount = 3
                    formatter = { financialNumber(it) }
                }
            }
            theme {
                backgroundColor = StockChatTheme.surface
                textColor = StockChatTheme.textSecondary
