    ...
    fun share(content: ShareContent, callback: (ShareResult) -> Unit) {
        ...
        toNative(
            ...
            callback = { result ->
                callback(
                    when {
                        result == null -> ShareResult.Failure(errorCode = "NO_RESPONSE", errorMessage = "分享暂时不可用，请稍后重试")
                        result.optInt("cancelled", 0) == 1 || result.optBoolean("cancelled") -> ShareResult.Cancelled
                        result.optInt("success", 0) == 1 || result.optBoolean("success") -> ShareResult.Success
                        else ->
                            ShareResult.Failure(
                                errorCode = result.optString("errorCode"),
                                errorMessage = result.optString("errorMessage").ifBlank { "分享失败，请稍后重试" },
                            )
                    }
                )
            },
