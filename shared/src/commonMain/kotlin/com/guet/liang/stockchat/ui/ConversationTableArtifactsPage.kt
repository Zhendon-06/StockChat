...
internal class ConversationTableArtifactsPage : BasePager() {
    ...
    private fun EmptyState(container: ViewContainer<*, *>) {
        with(container) {
            View {
                ...
                Text {
                    attr {
                        text("在聊天页右上角选择“产物表格”，即可把当前会话整理为结构化表格。")
                        fontSize(14f)
                        lineHeight(21f)
                        textAlignCenter()
                        color(StockChatTheme.textSecondary)
        ...
        with(container) {
            Scroller {
                ...
                View {
                    ...
                    Text {
                        attr {
                            text("按最近更新时间排列，进入详情后可左右滑动查看完整字段。")
                            fontSize(13f)
                            lineHeight(19f)
                            color(StockChatTheme.textSecondary)
                            marginTop(5f)
        ...
        with(container) {
            View {
                ...
                View {
                    ...
                    Text {
                        attr {
                            text(summary.title.ifBlank { "当前会话 · 产物表格" })
                            fontSize(16f)
                            fontWeightBold()
                            lineHeight(22f)
                            lines(2)
                    ...
                    Text {
                        attr {
                            text("共 ${summary.rowCount} 行 · 本地产物 #${summary.id}")
                            fontSize(12f)
    ...
    private fun loadArtifacts() {
        ...
        uiState = try {
            ...
            ArtifactListUiState.Error("本地产物暂时无法读取，请稍后重试。")
        }
    }
