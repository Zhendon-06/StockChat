...
internal fun detailPredictionConfig(configuration: ModelConfiguration, routeApiKey: String): StockPredictionConfig {
    ...
    return StockPredictionConfig(
        apiKey = key,
        baseUrl = provider?.baseUrl?.trim().orEmpty().ifBlank { if (dashScope) DEFAULT_CHAT_BASE_URL else "" },
