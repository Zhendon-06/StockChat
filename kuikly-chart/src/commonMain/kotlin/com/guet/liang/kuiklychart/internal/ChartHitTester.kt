...
internal object ChartHitTester {
    ...
    private fun cartesianSelectionAt(
        ...
        val bar = geometry.bars.firstOrNull { it.rect.contains(horizontal, vertical) }
        return if (bar != null) selection(spec, bar.seriesIndex, bar.dataIndex)
        else lineSelectionAt(spec, geometry, horizontal, vertical)
    }

    private fun lineSelectionAt(
        spec: ChartSpec,
        geometry: ChartRenderGeometry,
        horizontal: Float,
        vertical: Float,
    ...
    private fun pieSelectionAt(
        ...
        geometry.pieSlices.forEach { pieSlice ->
            ...
            if (angleFromStart <= sweep) {
                return selection(spec, pieSlice.seriesIndex, pieSlice.dataIndex)
            }
        }
        return null
    }
